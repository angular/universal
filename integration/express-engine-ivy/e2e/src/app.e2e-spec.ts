...
describe('Hello world E2E Tests', () => {
  ...
  it('should display: Hello world!', async () => {
    ...
    const style = browser.driver.findElement(by.css('style[ng-app-id="hlw"]'));
    ...
    expect(element(by.css('style[ng-app-id="hlw"]')).isPresent()).toBeFalsy();
  ...
  it('stylesheets should be configured to load asynchronously', async () => {
    ...
    // Test the contents from the server.
    const linkTag = browser.driver.findElement(by.css('link[rel="stylesheet"]'));
    expect(linkTag.getAttribute('media')).toMatch('all');
    expect(linkTag.getAttribute('onload')).toMatch(`this.media='all'`);
